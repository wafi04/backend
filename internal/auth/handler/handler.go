...
func (s *AuthHandler) GetUser(c *gin.Context) {
	...
	session, err := s.AuthService.AuthRepository.GetCurrentSession(c.Request.Context(), &authrepository.ReqSession{
		SessionID: sessionID,
	})
	if err != nil {
	...
	// Validasi bahwa session benar-benar milik user yang login
