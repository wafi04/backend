...
func (D *AuthRepository) UpdateSessionActivity(ctx context.Context, sessionID string) (*types.Session, error) {
	newExpiresAt := time.Now().Add(7 * 24 * time.Hour)

	var created_at, last_activity_at, expires_at time.Time
    ...
    WHERE session_id = $2
    RETURNING session_id, user_id, access_token, refresh_token, device_info, ip_address, created_at, expires_at, last_activity_at, is_active
    `
	var session types.Session
	err := D.DB.QueryRowContext(ctx, query, newExpiresAt, sessionID).Scan(
		&session.SessionID,
		&session.UserID,
		&session.AccessToken,
		&session.RefreshToken,
	...
	if err != nil {
		return nil, fmt.Errorf("failed to update session activity: %v", err)
	}

	session.CreatedAt = created_at.Unix()
	session.LastActivityAt = last_activity_at.Unix()
	session.ExpiresAt = expires_at.Unix()
